...
#region DllImport
    ...
    // standard sdk api
       ...
       [DllImport(MyLibName, CharSet = CharSet.Ansi)]
        protected static extern int createAgoraGameMediaEngine_gameEngine();

       [DllImport(MyLibName, CharSet = CharSet.Ansi)]
        protected static extern void initEventHandler_gameEngine(OnGMEngineRequestTokenHandler onGMEngineRequestToken,
                       OnEnterRoomSuccessHandler onEnterRoomSuccess,
                       OnEnterRoomFailHandler onEnterRoomFail,
                       OnConnectionStateChangeHandler onConnectionStateChange,
                       OnLostSyncHandler onLostSync, OnGetSyncHandler onGetSync,
                       OnTeamMateChangeHandler onTeamMateChange);
       
       [DllImport(MyLibName, CharSet = CharSet.Ansi)]
        protected static extern int initialize_gameEngine(string appId, uint areaCode);
       
       [DllImport(MyLibName, CharSet = CharSet.Ansi)]
        protected static extern void release_gameEngine();
       
       [DllImport(MyLibName, CharSet = CharSet.Ansi)]
        protected static extern int enableSpatializer_gameEngine(bool enable, bool applyToTeam);
       
       [DllImport(MyLibName, CharSet = CharSet.Ansi)]
        protected static extern int setRangeAudioTeamID_gameEngine(int teamID);
       
       [DllImport(MyLibName, CharSet = CharSet.Ansi)]
        protected static extern int setRangeAudioMode_gameEngine(int rangeAudioMode);
       
       [DllImport(MyLibName, CharSet = CharSet.Ansi)]
        protected static extern int setMaxHearAudioCount_gameEngine(int maxCount);
       
       [DllImport(MyLibName, CharSet = CharSet.Ansi)]
        protected static extern int setAudioRecvRange_gameEngine(int range);

       [DllImport(MyLibName, CharSet = CharSet.Ansi)]
        protected static extern int setDistanceUnit_gameEngine(float unit);
      
       [DllImport(MyLibName, CharSet = CharSet.Ansi)]
        protected static extern int updateSelfPosition_gameEngine(int[] position, float[] axisForward,
                         float[] axisRight, float[] axisUp);

       [DllImport(MyLibName, CharSet = CharSet.Ansi)]
        protected static extern int updateRemotePosition_gameEngine(uint uid, int[] position);
      
       [DllImport(MyLibName, CharSet = CharSet.Ansi)]
        protected static extern int setParameters_gameEngine(string parames);
       
       [DllImport(MyLibName, CharSet = CharSet.Ansi)]
        protected static extern int enterRoom_gameEngine(string token, string roomName, uint uid, SPACIAL_AUDIO_SYNC_MODE mode);
       
       [DllImport(MyLibName, CharSet = CharSet.Ansi)]
        protected static extern bool isRoomEntered_gameEngine();
       
       [DllImport(MyLibName, CharSet = CharSet.Ansi)]
        protected static extern int renewToken_gameEngine(string token);
       
       [DllImport(MyLibName, CharSet = CharSet.Ansi)]
        protected static extern int exitRoom_gameEngine();
       
       [DllImport(MyLibName, CharSet = CharSet.Ansi)]
        protected static extern int enableMic_gameEngine(bool enable);
       
       [DllImport(MyLibName, CharSet = CharSet.Ansi)]
        protected static extern int enableSpeaker_gameEngine(bool enable);
