...
/// <summary>
/// Author:WenJia
/// Description:选择UGC素材Item
/// Date: 2022/7/29 17:54:17
/// </summary>

public class UgcCreatInfo
{
    public NodeBaseBehaviour baseBehaviour;
    public string rId;
}

public class UgcPropChooseItem : MonoBehaviour
{
    public Button btnChooseProp;

    public Transform content;

    //已经选过的UGC,显示在panel下方供选择, key-rid
    public Dictionary<string, UgcChooseItem> UgcChooseItems = new Dictionary<string, UgcChooseItem>();

    //当前选中的item rid
    [HideInInspector]
    public string curChooseItemRid = string.Empty;
    [HideInInspector]
    public string UGC_CHOOSE_LOG = "UgcPropChooseItem:";
    [HideInInspector]
    public NodeBaseBehaviour curBehav;
    [HideInInspector]
    public List<string> allUsedUgcs;

    //上一次选择的item
    [HideInInspector]
    public UgcChooseItem lastChooseItem;
    private string currentChooseItem = string.Empty;

    public Action<NodeBaseBehaviour, string> AfterUgcCreateFinishCallback;

    public bool isChange = false;

    //是否在点击了Item的时候创建对应的物体
    public bool createItemObjWhenClick = true;
    //如果createItemObjWhenClick是false调用这个
    public Action<MapInfo> OnItemClickNotCreateCallBack;
    //是否要删除掉原来的占位物体
    public bool destroyBehav = true;
    //是否选中创建出来的新物体
    public bool selectCreateObj = true;

    public const int CHOOSE_TYPE_UGC = 0;
    public const int CHOOSE_TYPE_DC = 1;
    public int chooseType = CHOOSE_TYPE_UGC;
    ...
    /// </summary>
    public virtual void AfterUgcCreateFinish(NodeBaseBehaviour bev, string rId)
    {
    ...
    public void OnInitByCreate()
    {
        if (btnChooseProp != null)
        {
            btnChooseProp.onClick.AddListener(OpenUgcBagpackAndChoose);
        }

        RefreshUI();
    }

    public void SetEntity(NodeBaseBehaviour behaviour)
    {
        Debug.Log($"ErrorWatch Call SetEntity {gameObject.name} {behaviour.gameObject.name}");
        curBehav = behaviour;
        RefreshUI();
        InitUgcChoosePanel();
    }
    ...
    public void InitUgcChoosePanel()
    {
    ...
    public IEnumerator LoadResCoverTexture(MapInfo mapInfo)
    {
    ...
    public void ChooseItem(string rid)
    {
    ...
    public void CreateItem(MapInfo mapInfo)
    {
        if (!UgcChooseItems.ContainsKey(mapInfo.mapId))
        {
    ...
    public void CreateItemUI(MapInfo mapInfo, Action<UgcChooseItem> createCb)
    {
    ...
    /// </summary>
    public void OpenUgcBagpackAndChoose()
    {
        ...
        ResStorePanel.Show();
#else
        if (chooseType == CHOOSE_TYPE_UGC)
        {
            ResBagManager.Inst.OpenResPage(ChooseUgcCallback);
        }else if (chooseType == CHOOSE_TYPE_DC)
        {
            ResBagManager.Inst.OpenResPage(ChooseUgcCallback,OpenType.DC);
        }
#endif
    ...
    /// 从端上选择了UGC素材后回调
    /// </summary>
    public virtual void ChooseUgcCallback(MapInfo mapInfo)
    {
        ...
        if (createItemObjWhenClick)
        {
            CreateNewPropInSceneByUgcChoose(mapInfo, lastPos);
        }
        else
        {
            OnItemClickNotCreateCallBack?.Invoke(mapInfo);
        }
    ...
    public virtual void OnUgcChooseItemClick(UgcChooseItem item)
    {
        ...
        {
            OnItemClickNotCreateCallBack?.Invoke(item.mapInfo);
        }
    ...
    /// </summary>
    public void CreateNewPropInSceneByUgcChoose(MapInfo mapInfo, Vector3 pos, UgcChooseItem item = null)
    {
        ...
        if (nBehav)
        {
            ...
            //自定义操作
            AfterUgcCreateFinishCallback?.Invoke(nBehav, rId);
    ...
    public void CreateItemUISpecial(string IconPath,string key,Action ClickAction)
    {
        var itemPrefab = ResManager.Inst.LoadResNoCache<GameObject>("Prefabs/UI/Panel/WeaponItem");
        var item = Instantiate(itemPrefab, content, true);
        item.transform.SetAsFirstSibling();
        ...
        UgcChooseItems.Add(key, newItem);
    }

    public void AddSpecialItems(List<SpecialItem> defaultItems)
    {
        for (int i = 0; i < defaultItems.Count; i++)
        {
            SpecialItem item = defaultItems[i];
            CreateItemUISpecial( item.iconPath, item.key, () => { item.onClick(item.key);});
        }
    }

    /// <summary>
    /// 特殊的Item，可配置图片，key，和点击行为
    /// </summary>
