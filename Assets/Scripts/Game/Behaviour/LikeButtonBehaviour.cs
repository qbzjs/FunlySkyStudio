...
/// </summary>
public class LikeButtonBehaviour : NodeBaseBehaviour
{
    ...
    public override void OnInitByCreate()
    {
        ...
        mCube = this.gameObject.transform.Find("likebutton").Find("thumbsuptap").gameObject;
    ...
    private void OnChangeMode(GameMode mode)
    {
        ...
        if (mode == GameMode.Edit)
            SceneBuilder.Inst.UpdateAllLikeButton(0);
    }
    ...
    public override void OnRayEnter()
    {
        ...
        PortalPlayPanel.Instance.SetIcon(PortalPlayPanel.IconName.Like);
        PortalPlayPanel.Instance.AddButtonClick(OnClickLike);
    ...
    void OnClickLike()
    {
        ...
        {
            TipPanel.ShowToast("Liked(playtest)");
            SceneBuilder.Inst.UpdateAllLikeButton(1);
        }
    }

    private void DoRequestLike()
    {
        ...
        HttpUtils.MakeHttpRequest("/ugcmap/setLike", (int)HTTP_METHOD.POST, JsonConvert.SerializeObject(upLoadLikeBody), OnLikeSuccess, OnLikeFailed);
    }

    public void OnLikeSuccess(string msg)
    {
        //LoggerUtils.Log("OnLikeSuccess=>" + msg);
        TipPanel.ShowToast("Liked");
        SceneBuilder.Inst.UpdateAllLikeButton(1);
    }
    ...
    {
        if (GlobalFieldController.CurMapInfo != null && GlobalFieldController.CurMapInfo.interactStatus.liked == 1)
        {
            isCanClick = false;
            PortalPlayPanel.Hide();
            mAnimator.Play("push");
