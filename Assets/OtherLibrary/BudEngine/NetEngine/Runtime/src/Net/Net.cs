...
using BudEngine.NetEngine.src.Util.Def; // using System.Timers;
...
namespace BudEngine.NetEngine.src.Net {
    ...
    public class Net : IDisposable {
        ...
        protected static readonly ConcurrentDictionary<ServerSendClientBstType, BroadcastCallback> BroadcastHandlers = new ConcurrentDictionary<ServerSendClientBstType, BroadcastCallback> ();
        private static readonly Timer Timer = new Timer ();

        protected readonly ConcurrentDictionary<ServerSendClientBstType, Object> bdhandlers = new ConcurrentDictionary<ServerSendClientBstType, Object> ();
        // 该实例对象的发送队列
        ...
        // 设置广播回调
        private void SetBroadcastHandler (ServerSendClientBstType type, BroadcastCallback callback) {
        ...
        // 调用 Socket 发送消息
        protected string Send (byte[] data, string seq, ClientSendServerReqCmd subcmd) {
        ...
        private void HandleSendFail (string seq, int code)
        {
            ...
            switch (code) {
                case (int) ProtoErrCode.EcSdkUninit:
                    // 没有初始化
                    val.sendFail (code, null);
                    break;
                case (int) ProtoErrCode.EcSdkNoLogin:
                    // 没登录
                    Socket.Emit ("autoAuth", new SocketEvent ());
                    return;
                case (int) ProtoErrCode.EcSdkNoCheckLogin:
                    {
                        // 没checklogin
                        Socket.Emit ("autoAuth", new SocketEvent ());
                        return;
        ...
        private int GetReadyCode (ClientSendServerReqCmd subcmd) {
            ...
            if (Socket == null || string.IsNullOrEmpty (Socket.Url))
                return (int) ProtoErrCode.EcSdkSendFail;

            // 帧同步链接，不再依赖socket1链接上
            if (Socket.Id == (int) ConnectionType.Common && !UserStatus.IsStatus (UserStatus.StatusType.Login))
                return (int) ProtoErrCode.EcSdkNoLogin;

            if (Socket.Id == (int) ConnectionType.Relay && !CheckLoginStatus.IsChecked () &&
                (subcmd == ClientSendServerReqCmd.ECmdRelaySendFrameReq || subcmd == ClientSendServerReqCmd.ECmdRelayRequestFrameReq ||
                    subcmd == ClientSendServerReqCmd.ECmdHeartBeatReq || subcmd == ClientSendServerReqCmd.ECmdRelayClientSendtoGamesvrReq))
                return (int) ProtoErrCode.EcSdkNoCheckLogin;
