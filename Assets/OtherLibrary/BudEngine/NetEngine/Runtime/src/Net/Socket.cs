    ...
    namespace Sockets {
        public class Socket {
            ...
            private void OpenSocketTask (string tag) {
                Debugger.Log("Bud: OpenSocketTask tag:" + tag);
                // UnityEngine.Debug.Log("###websocket OpenSocketTask 线程ID:" + Thread.CurrentThread.ManagedThreadId);
            ...
            private void HandleSocketClose () {
                EmitCloseStatus ();
                //Debug.Log("Bud: OpenSocketTask HandleSocketClose");
            ...
            public void SetScocketStatus(int status)
            {
                if(_socketTask != null)
                {
                    _socketTask.ReadyState = status;
                }
            }
