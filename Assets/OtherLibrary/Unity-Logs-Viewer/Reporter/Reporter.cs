...
public class Reporter : MonoBehaviour
{
	...
	void Start()
	{
		logDate = System.DateTime.Now.ToString(System.Globalization.CultureInfo.InvariantCulture);
	...
	private float screenWidth13 = Screen.width / 3;
	private float screenHeight13 = Screen.height  / 3;
		...
		if (Application.platform == RuntimePlatform.Android ||
			...
			if (Input.touches.Length != 1) {
				...
				if (Input.touches[0].phase == TouchPhase.Canceled || Input.touches[0].phase == TouchPhase.Ended)
					...
					if (p.x < screenWidth13 || p.y < screenHeight13)
					{
						gestureDetector.Clear();
						return false;
					}
