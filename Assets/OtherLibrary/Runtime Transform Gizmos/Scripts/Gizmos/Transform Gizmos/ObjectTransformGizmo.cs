...
namespace RTG
{
    ...
    public class ObjectTransformGizmo : GizmoBehaviour
    {
        ...
        public void SetTargetObject(GameObject targetObject)
        {
            ...
            RefreshPositionAndRotation();
            IRTTransformGizmoListener transformGizmoListener = targetObject.GetComponent<IRTTransformGizmoListener>();
        ...
        private void ScaleObject(GameObject gameObject, Vector3 scalePivot)
        {
            ...
            var transformGizmoListener = gameObject.GetComponent<NodeBaseBehaviour>();
            if (transformGizmoListener != null && transformGizmoListener.norScale) 
                OnTransformed(lastScale,gameObject.transform,Gizmo);
        }

        private void OnTransformed(Vector3 lastScale,Transform transform, Gizmo transformGizmo)
        {
            if (transformGizmo.ScaleGizmo != null)
            {
                var scale = transform.localScale;
                if (transformGizmo.DragHandleId == GizmoHandleId.PXCap
                    || transformGizmo.DragHandleId == GizmoHandleId.PXSlider)
                {
                    scale.y = scale.x * lastScale.y / lastScale.x;
                    scale.z = scale.x * lastScale.z / lastScale.x;
                }
                else if (transformGizmo.DragHandleId == GizmoHandleId.PYCap
                         || transformGizmo.DragHandleId == GizmoHandleId.PYSlider)
                {
                    scale.x = scale.y * lastScale.x / lastScale.y;
                    scale.z = scale.y * lastScale.z / lastScale.y;
                }
                else if (transformGizmo.DragHandleId == GizmoHandleId.PZCap
                         || transformGizmo.DragHandleId == GizmoHandleId.PZSlider)
                {
                    scale.x = scale.z * lastScale.x / lastScale.z;
                    scale.y = scale.z * lastScale.y / lastScale.z;
                }
