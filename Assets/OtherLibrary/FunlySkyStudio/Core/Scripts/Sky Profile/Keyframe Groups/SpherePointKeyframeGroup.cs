...
namespace Funly.SkyStudio
{
  ...
  public class SpherePointKeyframeGroup : KeyframeGroup<SpherePointKeyframe>
  {
    ...
    public void SpherePointForTimeNoGc(float time, ref SpherePoint sp)
    {
      int beforeIndex;
      int afterIndex;

      // Shortcut and skip a calculations.
      if (keyframes.Count == 1 && sp != null)
      {
        sp.horizontalRotation = keyframes[0].spherePoint.horizontalRotation;
        sp.verticalRotation = keyframes[0].spherePoint.verticalRotation;
        return;
      }
      ...
      Vector2 coords = SphereUtility.DirectionToSphericalCoordinate(point.normalized);
      if (sp != null)
      {
        sp.horizontalRotation = coords.x;
        sp.verticalRotation = coords.y;
      }
