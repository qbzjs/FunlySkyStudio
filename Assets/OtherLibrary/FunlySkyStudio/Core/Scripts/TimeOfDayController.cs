...
namespace Funly.SkyStudio
{
    ...
    public class TimeOfDayController : MonoBehaviour
    {
        ...
        [Tooltip("Time is expressed in a fractional number of days that have completed.")] [SerializeField]
        ...
        {
            get { return m_SkyMaterialController; }
        }

        [Tooltip("Speed at which to advance time by if in automatic increment is enabled.")] [Range(0, 1)]
        public float automaticIncrementSpeed = .01f;

        [Tooltip("Sun orbit.")] public OrbitingBody sunOrbit;

        [Tooltip("Moon orbit.")] public OrbitingBody moonOrbit;
        ...
        public SpherePoint sunSpherePoint;
        public SpherePoint moonSpherePoint;
        ...
        [Tooltip(
            "Configurable prefab that determines how to animate between 2 sky profiles. You can override individual feature animations, ex: 'skyBlender', to create a custom sky blending effect.")]
        ...
        void Awake()
        {
            ...
            sunSpherePoint = new SpherePoint();
            moonSpherePoint = new SpherePoint();
        }
        ...
        {
            Debug.LogError(
                "Your SkySystemController has an old or invalid prefab layout! Please run the upgrade tool in 'Windows -> Sky Studio -> Upgrade Sky System Controller'. Do not rename or modify any of the children in the SkySystemController hierarchy.");
        }
        ...
        void Update()
        {
            ...
            #region Bud:控制同一时间只有一盏灯，限制时间每过1h进行计算，避免频繁调用

            if (SceneBuilder.Inst != null &&
                SceneBuilder.Inst.SkyboxBev != null &&
                SkyboxManager.Inst.GetCurSkyboxType() == SkyboxType.DayNight &&
                SceneBuilder.Inst.SkyboxBev.IsDayNightSkyRunning)
            {
                if (lastTimeOfDay == 0f)
                {
                    // LoggerUtils.Log($"fsc lastTimeOfDay init, timeOfDay:{timeOfDay}，小时：{timeOfDay*24}");
                    lastTimeOfDay = timeOfDay;
                    SetLightShow();
                }
                else if (timeOfDay < lastTimeOfDay)
                {
                    // LoggerUtils.Log($"fsc 过了一天, timeOfDay:{timeOfDay}，小时：{timeOfDay*24}");
                    lastTimeOfDay = timeOfDay;
                    SetLightShow();
                }
                else
                {
                    var passedTime = timeOfDay - lastTimeOfDay;
                    var passedHour = passedTime * 24;
                    if (passedHour > 1)
                    {
                        // LoggerUtils.Log($"fsc 过一小时 passedHour:{passedHour}, passedTime:{passedTime}, timeOfDay:{timeOfDay}，小时：{timeOfDay*24}");
                        lastTimeOfDay = timeOfDay;
                        SetLightShow();
                    }
                }
        ...
        #region Bud:计算并控制light显示

        private float lastTimeOfDay = 0f;

        public void ResetLastTimeOfDay()
        {
            lastTimeOfDay = 0f;
        }

        public void SetSunMoonLight(bool sunShow, bool moonShow)
        {
            // if (sunLight && sunLight.activeSelf != sunShow)
            // {
            //     sunLight.SetActive(sunShow);
            // }
            //
            // if (moonLight && moonLight.activeSelf != moonShow)
            // {
            //     moonLight.SetActive(moonShow);
            // }
        }

        public void SetLightShow()
        {
            // if ((timeOfDay >= 20 / 24f && timeOfDay <= 22 / 24f) ||
            //     (timeOfDay >= 1 / 24f && timeOfDay <= 3 / 24f))
            // {
            //     //20点到22--1点到3点 两盏灯打开，显示过渡效果
            //     SetSunMoonLight(true, true);
            // }
            // else if (timeOfDay > 3 / 24f && timeOfDay < 20 / 24f)
            // {
            //     //3点-20点显示白天光
            //     SetSunMoonLight(true, false);
            // }
            // else
            // {
            //     //22点-凌晨3点显示夜晚光
            //     SetSunMoonLight(false, true);
            // }
        }
        ...
        /// <summary>
        /// Start animating from one SkyProfile to another SkyProfile.
        /// </summary>
        /// <param name="toProfile"></param>
        /// <param name="duration"></param>
        public Boolean StartSkyProfileTransition(SkyProfile toProfile, float duration = 1.0f)
        {
            CancelSkyProfileTransition();

            if (skyProfileTransitionPrefab == null)
            {
            ...
            BlendSkyProfiles blender = Instantiate(skyProfileTransitionPrefab);
            blender.onBlendComplete += OnBlendComplete;

            SkyProfile blendingProfile = blender.StartBlending(this, skyProfile, toProfile, duration);
            if (blendingProfile == null)
            {
                Debug.LogWarning("Failed to create blending profile, check your from/to args.");
                return false;
            }

            // Assign this as our active profile, while blending is active.
            skyProfile = blendingProfile;

            return true;
        }

        /// <summary>
        /// Cancel any in progress profile animating blending, and immediately assign the final blending profile.
        /// </summary>
        public void CancelSkyProfileTransition()
        {
            BlendSkyProfiles blender = GetComponent<BlendSkyProfiles>();
            if (blender == null)
            {
                return;
            }

            blender.CancelBlending();
        }

        public void OnBlendComplete(BlendSkyProfiles blender)
        {
            // After blending we set the profile, and start time at zero.
            skyProfile = blender.toProfile;
            skyTime = 0;
        }

        /// <summary>
        /// Check if profile blending is currently in progress.
        /// </summary>
        /// <returns></returns>
        public bool IsBlendingInProgress()
        {
            return GetComponent<BlendSkyProfiles>() ? true : false;
        }
        ...
        public void UpdateSkyForCurrentTime()
        {
            ...
            // Ambient Light.
            if (skyProfile.IsFeatureEnabled(ProfileFeatureKeys.AmbientLightGradient))
            {
                if (RenderSettings.ambientMode != AmbientMode.Trilight)
                {
                    Debug.Log(
                        "Sky Profile is using Ambient Light feature, however Unity scene isn't configured for environment gradient. Changing environment to trilight gradient...");
                    RenderSettings.ambientMode = AmbientMode.Trilight;
                }

                RenderSettings.ambientSkyColor = skyProfile.GetColorPropertyValue(ProfilePropertyKeys.AmbientLightSkyColorKey, timeOfDay);
                RenderSettings.ambientEquatorColor = skyProfile.GetColorPropertyValue(ProfilePropertyKeys.AmbientLightEquatorColorKey, timeOfDay);
                RenderSettings.ambientGroundColor = skyProfile.GetColorPropertyValue(ProfilePropertyKeys.AmbientLightGroundColorKey, timeOfDay);
            }

            // Clouds.
            // if (skyProfile.IsFeatureEnabled(ProfileFeatureKeys.CloudFeature))
            // {
            //   m_SkyMaterialController.CloudAlpha = skyProfile.GetNumberPropertyValue(ProfilePropertyKeys.CloudAlpha, timeOfDay);
            //
            ...
            if (skyProfile.IsFeatureEnabled(ProfileFeatureKeys.SunFeature) && sunOrbit)
            {
                ...
                // bud gc优化--原GetSpherePointPropertyValue带来96B GC
                skyProfile.GetSpherePointPropertyValueNoGc(ProfilePropertyKeys.SunPositionKey, timeOfDay, ref sunSpherePoint);
                sunOrbit.Point = sunSpherePoint;
                ...
                m_SkyMaterialController.SunAlpha = skyProfile.GetNumberPropertyValue(ProfilePropertyKeys.SunAlpha, timeOfDay);
            ...
            if (skyProfile.IsFeatureEnabled(ProfileFeatureKeys.MoonFeature) && moonOrbit)
            {
                ...
                // bud gc优化--原GetSpherePointPropertyValue带来96B GC
                skyProfile.GetSpherePointPropertyValueNoGc(ProfilePropertyKeys.MoonPositionKey, timeOfDay, ref moonSpherePoint);
                moonOrbit.Point = moonSpherePoint;
                ...
                m_SkyMaterialController.MoonAlpha = skyProfile.GetNumberPropertyValue(ProfilePropertyKeys.MoonAlpha, timeOfDay);
