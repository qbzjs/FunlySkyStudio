...
namespace Funly.SkyStudio
{
  ...
  public class SkySetupWindow : EditorWindow
  {
    ...
    private ProfilePreset m_SelectedProfilePreset;
    private List<ProfilePreset> m_Presets = new List<ProfilePreset>();
    ...
    private void OnGUI()
    {
      ...
      RenderPresetPopup(m_Presets);

      EditorGUILayout.Space();
      EditorGUILayout.Space();

      if (GUILayout.Button(new GUIContent("Create Sky System"))) {
        SetupSceneWithPreset(m_SelectedProfilePreset);
      }
    }
    ...
    private void RenderPresetPopup(List<ProfilePreset> presets)
    {
      ...
      for (int i = 0; i < presets.Count; i++) {
        ...
        if (m_SelectedProfilePreset != null && preset.assetPath == m_SelectedProfilePreset.assetPath) {
          selectedIndex = i;
        }
      }

      selectedIndex = EditorGUILayout.Popup("Sky Preset", selectedIndex, displayedPrests.ToArray());
      m_SelectedProfilePreset = presets[selectedIndex];
    }
    ...
    private List<ProfilePreset> LoadListOfPresets()
    {
      ...
      foreach (string guid in guids) {
        ...
        if (menuName.Contains(SkyEditorUtility.SkyStudioRootDirectory()))
        {
          menuName = "Presets/" + presetName;
        }
        else
        {
