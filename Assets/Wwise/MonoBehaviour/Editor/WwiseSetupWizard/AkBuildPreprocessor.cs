...
#endif
{
	...
	public void OnPostprocessBuildInternal(UnityEditor.BuildTarget target, string path)
	{
		...
		//jaywill-20220207:打包后自动删除非当前平台的wwise资源（如：当前为Android 则删除ios相关wwise资源）-start
		string androidPath = System.IO.Path.Combine(path,"unityLibrary","src","main","assets","Audio","GeneratedSoundBanks");
		string iosPath = System.IO.Path.Combine(path,"Data","Raw","Audio","GeneratedSoundBanks");
		...
		if(System.IO.Directory.Exists(dstPath)){
			string[] dirs2  = System.IO.Directory.GetDirectories(dstPath);
			for(int i = 0;i < dirs2.Length;i++){
				if(!dirs2[i].Contains(platformName)){
					DeleteSoundbanks(dirs2[i]);
				}
			}
		}else{
			UnityEngine.Debug.LogErrorFormat("WwiseUnity AkBuildPreprocessor DeleteSoundbanks path<{0}> is not exists!",dstPath);
		}	
		//jaywill-20220207:打包后自动删除非当前平台的wwise资源（如：当前为Android 则删除ios相关wwise资源）-end
	}
